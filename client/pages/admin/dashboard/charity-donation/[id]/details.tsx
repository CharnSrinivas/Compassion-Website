...
export async function getServerSideProps(context: GetServerSidePropsContext): Promise<GetServerSidePropsResult<Record<string, unknown>>> {
    ...
    const query = qs.stringify({
        ...
        }, populate: ['charity', 'user']
    });

    const donation = await (await fetch(server_url + "/content-manager/collection-types/api::charity-donation.charity-donation?" + query, {
        method: "GET",
        headers: {
            Authorization: `Bearer ${admin_token}`,
        }
...
export default function edit({ donation, admin_token, id }: Props) {
    ...
    const _delete = async () => {
        try {
            setDeleting(true);
            let res = await fetch(server_url + "/content-manager/collection-types/api::charity-donation.charity-donation/" + donation.id, {
                method: "DELETE", headers: {
                    Authorization: `Bearer ${admin_token}`,
                }
            });
            window.location.href = '/admin/dashboard/charity-donation'
            setDeleting(false);
        } catch (error) {
            setDeleting(false);
        }
                        ...
                        <div className="lg:w-1/2 w-full lg:pr-10 lg:py-6 mb-6 lg:mb-0">
                            ...
                            <div>
                                ...
                                <div className="flex border-t border-gray-200 py-2">
                                    ...
                                    <a href={`/admin/dashboard/charity/${donation['charity']['slug']}`} className="ml-auto text-blue-800">
                                        {donation['charity']['name']}
                                    </a>
                                </div>
                                ...
                                <div className="flex border-t border-gray-200 py-2">
                                    ...
                                    <span className="ml-auto text-gray-900" style={{ lineBreak: "anywhere" }} >{donation['amount']} <strong className='uppercase text-gray-600'>{donation['charity']['fund_type']}</strong> </span>
                                </div>
                                <div className="flex border-t border-gray-200 py-2">
                                ...
                                <h2 className='font-medium mt-8'>Word of Support</h2>
                            ...
                            <div className='flex flex-wrap gap-4'>
                                {!donation.success &&
                                    <button onClick={toggleApproval} disabled={donation.success} className={`flex gap-1  items-center text-white  bg-green-500 hover:bg-green-600   border-0 py-2 px-4 focus:outline-none rounded `}>
                                        Approve
                                        {approving &&
                                            <svg
