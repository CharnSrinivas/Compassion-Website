...
export default function story({  charity, is_auth, token }: Props) {
    const router = useRouter();
    const [description, setDescription] = useState(charity.attributes.description);
    ...
    const submit = async () => {
        ...
        if (res.ok) {
            router.push(`/create/charity/${charity.attributes.slug}/add-image`);
            return;
        }
        setSubmitting(false)
    }
    ...
    return (
        ...
        <div className="font-medium m-auto text-4xl text-green-900 mt-7 text-center">Description</div>
        <p className=" text-gray-500 text-center mt-3"> Explain the purpose of the charity.</p>
